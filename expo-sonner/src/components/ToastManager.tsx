...
class ToastManagerComponent extends Component<ToastManagerProps, ToastState> {
  ...
  getPositionStyle = (): object => {
    ...
    switch (position) {
      case "top":
        return { top: topOffset || 40 };
      case "bottom":
        return { bottom: bottomOffset || 40 };
      case "center":
  ...
  renderToastContent = (): ReactNode => {
    ...
    const finalIconSize =
      stateIconSize !== undefined ? stateIconSize : propsIconSize;

    // Use icon from state if provided, otherwise get icon based on type
    const finalIcon =
      stateIcon !== undefined ? stateIcon : this.getIconForType(type);
    ...
    const finalCloseIcon =
      stateCloseIcon !== undefined ? stateCloseIcon : propsCloseIcon;
    const finalCloseIconSize =
      stateCloseIconSize !== undefined
        ? stateCloseIconSize
        : propsCloseIconSize;
    const finalCloseIconColor =
      stateCloseIconColor !== undefined
        ? stateCloseIconColor
        : propsCloseIconColor;
    const finalCloseIconFamily =
      stateCloseIconFamily || propsCloseIconFamily || "Ionicons";
...
(ToastManager as ToastManagerType).success = (
  text: string,
...
(ToastManager as ToastManagerType).error = (
  text: string,
  position?: ToastPosition,
) => {
  ToastManagerComponent.toastRef?.current?.show({
...
(ToastManager as ToastManagerType).info = (
  text: string,
  position?: ToastPosition,
) => {
  ToastManagerComponent.toastRef?.current?.show({
...
(ToastManager as ToastManagerType).warn = (
  text: string,
  position?: ToastPosition,
) => {
  ToastManagerComponent.toastRef?.current?.show({
...
(ToastManager as ToastManagerType).defaultProps =
  ToastManagerComponent.defaultProps;
