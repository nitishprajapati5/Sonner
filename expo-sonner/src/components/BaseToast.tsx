...
const loadIconFamily = (familyName: string) => {
  try {
    ...
    console.warn(
      `Icon family ${familyName} not found. Please install react-native-vector-icons or use custom icons.`,
    );
    return null;
  }
};
...
const BaseToast = ({
  ...
  const rtlContentStyle = isRTL
    ? { flexDirection: "row-reverse" as "row-reverse" }
    : {};
  const rtlHideButtonStyle = isRTL
    ? { right: undefined, left: SCALE(3.2) }
    : {};
  const rtlIconWrapperStyle = isRTL
    ? { marginRight: 0, marginLeft: SCALE(8) }
    : {};
  ...
  const renderCloseIcon = () => {
      ...
      if (!CloseIconComponent) {
        return (
          <View style={styles.closeFallback}>
            <Text
              style={{ color: finalCloseIconColor, fontSize: closeIconSize }}
            >
              ×
            </Text>
          </View>
    ...
    if (!DefaultCloseIconComponent) {
      return (
        <View style={styles.closeFallback}>
          <Text style={{ color: finalCloseIconColor, fontSize: closeIconSize }}>
            ×
          </Text>
        </View>
      );
